...
- *"Product names must be unique per seller."* The tempting answer is to make Seller a big aggregate containing all its products, so the invariant sits inside one boundary. Resist it: that turns every product operation into a load-all-products operation, and two sellers adding products concurrently now contend on one aggregate. My answer: keep the aggregates small and enforce uniqueness with a database unique index on `(seller_id, name)` — a deliberate, documented exception where infrastructure enforces a domain rule, because it does so atomically and cheaply. Dogma loses to a unique index.
